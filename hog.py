...
def is_prime(current_score):
    ...
    non_prime_factors = 0
    x = 1
    while x <= current_score:
        if x != 1 and x != current_score and current_score % x == 0:
            non_prime_factors += 1
        x += 1
    if non_prime_factors > 0 or current_score == 1:
        return False
    else:
        return True
...
def play(strategy0, strategy1, score0=0, score1=0, goal=GOAL_SCORE):
    ...
    while score0 < goal and score1 < goal:
        ...
        if player == 0:
            num_dice_0 = strategy0(score0, score1)
            if num_dice_0 == -1:
                dice_swapped = not dice_swapped
                score0 += 1
            else:
                score0 += take_turn(num_dice_0, score1, dice)
            if score0 == score1 * 2 or score1 == score0 * 2:
                score0, score1 = score1, score0
        else:
            num_dice_1 = strategy1(score1, score0)
            if num_dice_1 == -1:
                dice_swapped = not dice_swapped
                score1 += 1
            else:
                score1 += take_turn(num_dice_1, score0, dice)
            if score0 == score1 * 2 or score1 == score0 * 2:
                score0, score1 = score1, score0
        player = other(player)
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    for num_rolls in range(1, 11):
        averages.append(average(num_rolls, dice))
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    # BEGIN PROBLEM 10
    free_bacon_score = free_bacon(opponent_score)
    total_score = score + free_bacon_score
    if opponent_score == 2 * total_score:
        return 0
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    """
    # BEGIN PROBLEM 11
    num_dice = swap_strategy(score, opponent_score)
    if score == 0 or score == (opponent_score/2) - 1:
        num_dice = -1
    if 100-score <= 10:
        num_dice = bacon_strategy(score, opponent_score, 100-score)
    if (score + 1 + opponent_score) % 7 == 0:
        num_dice = bacon_strategy(score, opponent_score, 4, 8)
    return num_dice
